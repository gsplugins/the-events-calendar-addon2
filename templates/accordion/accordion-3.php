...
<?php else : ?>
	<div class="teca-accordion teca-accordion-3">
		<?php foreach ( $events as $event ) : ?>
			<?php
			$event_id   = (int) ( $event['event_id'] ?? 0 );
			$is_first   = 0 === $index;
			$classes    = array( 'teca-accordion-item', 'teca-accordion-3-item' );
			...
			$panel_id   = 'teca-accordion-3-panel-' . sanitize_key( (string) $id ) . '-' . $event_id;
			$trigger_id = 'teca-accordion-3-trigger-' . sanitize_key( (string) $id ) . '-' . $event_id;
			...
			<article class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>"<?php echo teca_get_events_section_item_attributes_html( $event, '' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
				<button
					...
					class="teca-accordion-trigger teca-accordion-3-trigger"
					type="button"
				...
				>
					<?php if ( ! empty( $date_parts['day'] ) || ! empty( $date_parts['month'] ) ) : ?>
						...
						<span class="teca-accordion-3-ticket-date gs-teca-date teca-event-date">
							<?php if ( ! empty( $date_parts['day'] ) ) : ?>
								<span class="teca-accordion-3-day"><?php echo esc_html( $date_parts['day'] ); ?></span>
							<?php endif; ?>
							<?php if ( ! empty( $date_parts['month'] ) ) : ?>
								<span class="teca-accordion-3-month"><?php echo esc_html( $date_parts['month'] ); ?></span>
							<?php endif; ?>
						</span>
						<?php endif; ?>
					<?php endif; ?>

					<span class="teca-accordion-3-ticket-main">
						<?php if ( teca_is_card_field_visible( 'event_title', $visibility_settings ?? null ) && $title ) : ?>
							<span class="teca-accordion-3-title gs-teca-title teca-event-title"><?php echo esc_html( $title ); ?></span>
						<?php endif; ?>
						<?php if ( $meta ) : ?>
							<span class="teca-accordion-3-meta gs-teca-date teca-event-date"><?php echo esc_html( $meta ); ?></span>
						<?php endif; ?>
					</span>

					<span class="teca-accordion-3-action"><?php esc_html_e( 'Details', 'the-events-calendar-addon2' ); ?></span>
				</button>

				<div
					id="<?php echo esc_attr( $panel_id ); ?>"
					class="teca-accordion-panel teca-accordion-3-panel"
					role="region"
