...
foreach ( $events as $event ) :
	...
	<div class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>"<?php echo teca_get_event_filter_attributes_html( $event ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
		...
		<article class="teca-grid-style-7-card gs-teca-event-main">

			<?php if ( Helpers::is_visible( $visibility_settings['event_thumbnail'] ?? true ) ) : ?>
				<div class="teca-grid-style-7-thumb">
					<div class="<?php Helpers::print_visible_classes( $visibility_settings['event_thumbnail'], 'teca-grid-style-7-thumb-inner teca-event-thumb gs-teca-thumbnail-wrapper' ); ?>">
						<?php include Template_Loader::locate_template( 'partials/gs-teca-event-thumbnail.php' ); ?>
					</div>
					<div class="teca-grid-style-7-thumb-overlay" aria-hidden="true"></div>

					<div class="teca-grid-style-7-thumb-badges">
						<?php if ( Helpers::is_visible( $visibility_settings['event_cat'] ?? true ) ) : ?>
							<div class="<?php Helpers::print_visible_classes( $visibility_settings['event_cat'], 'teca-grid-style-7-tax-row teca-event-categories gs-teca-categories' ); ?>">
			...
			<div class="teca-grid-style-7-content">

				<?php if ( Helpers::is_visible( $visibility_settings['event_title'] ?? true ) ) : ?>
					<div class="<?php Helpers::print_visible_classes( $visibility_settings['event_title'], 'teca-grid-style-7-title teca-event-title gs-teca-title' ); ?>">
				...
				<?php if ( $has_meta ) : ?>
					<div class="teca-grid-style-7-meta teca-event-meta">
						<?php if ( $time_display ) : ?>
							<span class="teca-grid-style-7-time teca-event-time"><?php echo esc_html( $time_display ); ?></span>
						<?php endif; ?>

						<?php if ( Helpers::is_visible( $visibility_settings['event_venue'] ?? true ) ) : ?>
							<div class="<?php Helpers::print_visible_classes( $visibility_settings['event_venue'], 'teca-grid-style-7-venue teca-event-venue gs-teca-venue' ); ?>">
								<?php
						...
						<?php if ( Helpers::is_visible( $visibility_settings['event_organizer'] ?? true ) ) : ?>
							<div class="<?php Helpers::print_visible_classes( $visibility_settings['event_organizer'], 'teca-grid-style-7-organizer teca-event-organizer gs-teca-organizer' ); ?>">
								<?php
								include Template_Loader::locate_template(
				...
				<?php if ( Helpers::is_visible( $visibility_settings['event_details'] ?? true ) ) : ?>
					<div class="<?php Helpers::print_visible_classes( $visibility_settings['event_details'], 'teca-grid-style-7-excerpt teca-event-excerpt gs-teca-desc' ); ?>">
				...
				teca_echo_google_calendar_button_actions(
					...
					'teca-grid-style-7-google-calendar-wrap',
...
<?php
				...
				if ( $show_footer_tags || $show_footer_link ) :
					?>
					<div class="teca-grid-style-7-footer">
						<?php if ( $show_footer_tags ) : ?>
							<div class="<?php Helpers::print_visible_classes( $visibility_settings['event_tags'], 'teca-grid-style-7-tags teca-event-tags gs-teca-tag' ); ?>">
								<?php include Template_Loader::locate_template( 'partials/gs-teca-tag.php' ); ?>
							</div>
						<?php endif; ?>

						<?php if ( $show_footer_link ) : ?>
							<div class="<?php teca_print_card_visible_classes( 'view_details_button', 'teca-grid-style-7-action gs-teca-view-details', $visibility_settings ?? null ); ?>">
								<?php
								echo wp_kses_post(
									teca_get_view_details_button_html(
										...
										array(
											...
											'button_class' => 'teca-event-button teca-grid-style-7-button',
