...
foreach ( $events as $event ) :
	...
	$show_date_badge = $day_label && Helpers::is_visible( $visibility_settings['event_date'] ?? true );
	$show_top_cat    = Helpers::is_visible( $visibility_settings['event_cat'] ?? true );
	$show_topbar     = $show_date_badge || $show_top_cat;
	...
	<div class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>"<?php echo teca_get_event_filter_attributes_html( $event ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
		...
		<article class="teca-grid-style-10-card gs-teca-event-main<?php echo $has_thumbnail ? '' : ' teca-grid-style-10-card--no-media'; ?>">

			<?php if ( $has_thumbnail ) : ?>
				<div class="teca-grid-style-10-media">
					<div class="<?php Helpers::print_visible_classes( $visibility_settings['event_thumbnail'], 'teca-grid-style-10-thumb teca-event-thumb gs-teca-thumbnail-wrapper' ); ?>">
						<?php include Template_Loader::locate_template( 'partials/gs-teca-event-thumbnail.php' ); ?>
					</div>
					<div class="teca-grid-style-10-media-overlay" aria-hidden="true"></div>
				</div>
			<?php endif; ?>
			...
			<?php if ( $show_topbar ) : ?>
				<div class="teca-grid-style-10-topbar">
					...
					<?php if ( $show_top_cat ) : ?>
						<div class="<?php Helpers::print_visible_classes( $visibility_settings['event_cat'], 'teca-grid-style-10-category teca-event-categories gs-teca-categories' ); ?>">
			...
			<div class="teca-grid-style-10-body">
				<?php if ( Helpers::is_visible( $visibility_settings['event_title'] ?? true ) ) : ?>
					<div class="<?php Helpers::print_visible_classes( $visibility_settings['event_title'], 'teca-grid-style-10-title teca-event-title gs-teca-title' ); ?>">
				...
				<?php if ( Helpers::is_visible( $visibility_settings['event_details'] ?? true ) ) : ?>
					<div class="<?php Helpers::print_visible_classes( $visibility_settings['event_details'], 'teca-grid-style-10-excerpt teca-event-excerpt gs-teca-desc' ); ?>">
			...
			<?php if ( $has_meta ) : ?>
				<div class="teca-grid-style-10-meta-panel">
					<div class="teca-grid-style-10-meta teca-event-meta">
						<?php if ( $time_display && Helpers::is_visible( $visibility_settings['event_date'] ?? true ) ) : ?>
							<span class="teca-grid-style-10-time teca-event-time"><?php echo esc_html( $time_display ); ?></span>
						<?php endif; ?>

						<?php if ( Helpers::is_visible( $visibility_settings['event_venue'] ?? true ) ) : ?>
							<div class="<?php Helpers::print_visible_classes( $visibility_settings['event_venue'], 'teca-grid-style-10-venue teca-event-venue gs-teca-venue' ); ?>">
								<?php
						...
						<?php if ( Helpers::is_visible( $visibility_settings['event_organizer'] ?? true ) ) : ?>
							<div class="<?php Helpers::print_visible_classes( $visibility_settings['event_organizer'], 'teca-grid-style-10-organizer teca-event-organizer gs-teca-organizer' ); ?>">
								<?php
								include Template_Loader::locate_template(
			...
			teca_echo_google_calendar_button_actions(
				...
				'teca-grid-style-10-calendar-action',
...
<?php
			...
			if ( $show_footer_tags || $show_footer_link ) :
				?>
				<div class="teca-grid-style-10-footer">
					<?php if ( $show_footer_tags ) : ?>
						<div class="<?php Helpers::print_visible_classes( $visibility_settings['event_tags'], 'teca-grid-style-10-tags teca-event-tags gs-teca-tag' ); ?>">
							<?php include Template_Loader::locate_template( 'partials/gs-teca-tag.php' ); ?>
						</div>
					<?php endif; ?>

					<?php if ( $show_footer_link ) : ?>
						<div class="<?php teca_print_card_visible_classes( 'view_details_button', 'teca-grid-style-10-action gs-teca-view-details', $visibility_settings ?? null ); ?>">
							<?php
							echo wp_kses_post(
								teca_get_view_details_button_html(
									...
									array(
										...
										'button_class' => 'teca-event-button teca-grid-style-10-button',
