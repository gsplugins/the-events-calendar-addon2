<?php
namespace GS_TECA;


/**
 * Protect direct access
 */
if ( ! defined( 'ABSPATH' ) ) exit;

/**
 * Handle asset loading through out the plugin.
 * 
 * @since 1.0.0
 */
final class Scripts {
	
	/**
	 * Contains styles handlers and paths.
	 *
	 * @since 1.0.0
	 */
	public $styles = [];

	/**
	 * Contains scripts handlers and paths.
	 *
	 * @since 1.0.0
	 */
	public $scripts = [];

	/**
	 * Class constructor.
	 *
	 * @since 1.0.0
	 */
	public function __construct() {
		add_action( 'plugins_loaded', [ $this, 'add_assets' ], 20 );
		add_action( 'wp_enqueue_scripts', [ $this, 'enqueue_scripts' ] );
		add_action( 'admin_enqueue_scripts', [ $this, 'enqueue_admin_scripts' ], 9999 );
		add_action( 'admin_head', [ $this, 'print_plugin_icon_css' ] );
		$this->register_optimization_exclusions();

		return $this;
	}

	/**
	 * Exclude TECA/Swiper from defer-delay-combine optimizers (demo-site breakage).
	 */
	public function register_optimization_exclusions() {
		$handles = [ 'gs-teca-public', 'gs-swiper', 'gs-carousel-ticker' ];

		add_filter( 'rocket_delay_js_exclusions', function( $excluded ) use ( $handles ) {
			return array_merge( (array) $excluded, $handles );
		} );

		add_filter( 'rocket_exclude_defer_js', function( $excluded ) use ( $handles ) {
			return array_merge( (array) $excluded, $handles );
		} );

		add_filter( 'rocket_exclude_js', function( $excluded ) use ( $handles ) {
			return array_merge( (array) $excluded, $handles );
		} );

		add_filter( 'litespeed_optimize_js_excludes', function( $excluded ) use ( $handles ) {
			return array_merge( (array) $excluded, $handles );
		} );

		add_filter( 'script_loader_tag', function( $tag, $handle ) use ( $handles ) {
			if ( ! in_array( $handle, $handles, true ) ) {
				return $tag;
			}

			if ( false !== strpos( $tag, 'data-no-optimize' ) ) {
				return $tag;
			}

			return str_replace( ' src=', ' data-no-optimize="1" src=', $tag );
		}, 10, 2 );
	}

	/**
	 * Adding assets on the $this->styles[] array.
	 *
	 * @since 1.0.0
	 */
	public function add_assets() {
		// Styles
		$this->add_style( 'gs-bootstrap-grid', GS_TECA_PLUGIN_URI . 'assets/libs/bootstrap-grid/bootstrap-grid.min.css', [], GS_TECA_VERSION );
		$this->add_style( 'gs-swiper', GS_TECA_PLUGIN_URI . 'assets/libs/swiper-js/swiper.min.css', [], GS_TECA_VERSION );
		$this->add_style( 'gs-tippyjs', GS_TECA_PLUGIN_URI . 'assets/libs/tippyjs/tippy.css', [], GS_TECA_VERSION );
		$this->add_style( 'gs-magnific-popup', GS_TECA_PLUGIN_URI . 'assets/libs/magnific-popup/magnific-popup.min.css', [], GS_TECA_VERSION );
		$this->add_style( 'gs-font-awesome-5', GS_TECA_PLUGIN_URI . 'assets/libs/font-awesome/css/font-awesome.min.css', [], GS_TECA_VERSION, 'all');
		$this->add_style( 'gs-teca-sort', GS_TECA_PLUGIN_URI . 'assets/admin/css/gs-teca-sort.min.css', [], GS_TECA_VERSION );
		
		// Scripts
		$this->add_script( 'gs-swiper', GS_TECA_PLUGIN_URI . 'assets/libs/swiper-js/swiper.min.js', ['jquery'], GS_TECA_VERSION, true );
		$this->add_script( 'gs-isotope', GS_TECA_PLUGIN_URI . 'assets/libs/isotope/isotope.min.js', ['jquery'], GS_TECA_VERSION, true );
		$this->add_script( 'gs-images-loaded', GS_TECA_PLUGIN_URI . 'assets/libs/images-loaded/images-loaded.min.js', ['jquery'], GS_TECA_VERSION, true );
		$this->add_script( 'gs-tippyjs', GS_TECA_PLUGIN_URI . 'assets/libs/tippyjs/tippy-bundle.umd.min.js', [], GS_TECA_VERSION, true );
		$this->add_script( 'gs-magnific-popup', GS_TECA_PLUGIN_URI . 'assets/libs/magnific-popup/magnific-popup.min.js', ['jquery'], GS_TECA_VERSION, true );
		$this->add_script( 'gs-carousel-ticker', GS_TECA_PLUGIN_URI . 'assets/libs/carouselTicker/carouselTicker.min.js', ['jquery'], GS_TECA_VERSION, true );
		$this->add_script( 'gs-teca-sort', GS_TECA_PLUGIN_URI . 'assets/admin/js/gs-teca-sort.min.js', ['jquery', 'jquery-ui-sortable'], GS_TECA_VERSION, true );

		$public_css     = GS_TECA_PLUGIN_DIR . 'assets/css/gs-teca.min.css';
		$public_js      = GS_TECA_PLUGIN_DIR . 'assets/js/gs-teca.min.js';
		$public_css_ver = file_exists( $public_css ) ? (string) filemtime( $public_css ) : GS_TECA_VERSION;
		$public_js_ver  = file_exists( $public_js ) ? (string) filemtime( $public_js ) : GS_TECA_VERSION;

		$this->add_style(
			'gs-teca-public',
			GS_TECA_PLUGIN_URI . 'assets/css/gs-teca.min.css',
			[ 'gs-bootstrap-grid', 'gs-font-awesome-5' ],
			$public_css_ver
		);
		$this->add_script(
			'gs-teca-public',
			GS_TECA_PLUGIN_URI . 'assets/js/gs-teca.min.js',
			[ 'jquery', 'gs-images-loaded', 'gs-magnific-popup', 'gs-carousel-ticker', 'gs-tippyjs', 'gs-isotope', 'gs-swiper' ],
			$public_js_ver,
			true
		);
		
		// For Divi fix
		$this->add_style( 'gs-teca-divi-public', GS_TECA_PLUGIN_URI . 'assets/css/gs-teca-divi.min.css', ['gs-teca-public'], GS_TECA_VERSION );

		do_action( 'gs_teca__add_assets', $this );

	}

	/**
	 * Store styles on the $this->styles[] queue.
	 * 
	 * @since 1.0.0
	 * 
	 * @param  string  $handler Name of the stylesheet.
	 * @param  string  $src     Full URL of the stylesheet
	 * @param  array   $deps    Array of registered stylesheet handles this stylesheet depends on.
	 * @param  boolean $ver     Specifying stylesheet version number
	 * @param  string  $media   The media for which this stylesheet has been defined.
	 * @return void
	 */
	public function add_style( $handler, $src, $deps = [], $ver = false, $media ='all' ) {
		$this->styles[$handler] = [
			'src' => $src,
			'deps' => $deps,
			'ver' => $ver,
			'media' => $media
		];
	}

	/**
	 * Store scripts on the $this->scripts[] queue.
	 * 
	 * @since 1.0.0
	 * 
	 * @param  string  $handler  Name of the script.
	 * @param  string  $src      Full URL of the script
	 * @param  array   $deps      Array of registered script handles this script depends on.
	 * @param  boolean $ver       Specifying script version number
	 * @param  boolean $in_footer Whether to enqueue the script before </body> instead of in the <head>
	 * @return void
	 */
	public function add_script( $handler, $src, $deps = [], $ver = false, $in_footer = false ) {
		$this->scripts[$handler] = [
			'src' => $src,
			'deps' => $deps,
			'ver' => $ver,
			'in_footer' => $in_footer
		];
	}

	/**
	 * Return style if exits on the $this->styles[] list.
	 * 
	 * @since 3.0.9
	 * @param string $handler The style name.
	 */
	public function get_style( $handler ) {
		if ( empty( $style = $this->styles[$handler] ) ) {
			return false;
		}

		return $style;
	}

	/**
	 * Return the script if exits on the $this->scripts[] list.
	 * 
	 * @since 3.0.9
	 * @param string $handler The script name.
	 */
	public function get_script( $handler ) {
		if ( empty( $script = $this->scripts[$handler] ) ) {
			return false;
		}

		return $script;
	}

	/**
	 * A wrapper for registering styles.
	 * 
	 * @since 1.0.0
	 * 
	 * @param  string       $handler The name of the stylesheet.
	 * @return boolean|void          If it gets the stylesheet then register it or false.
	 */
	public function wp_register_style( $handler ) {
		$style = $this->get_style( $handler );

		if ( ! $style ) {
			return;
		}

		$deps = (array) apply_filters( $handler . '--style', $style['deps'] );
		wp_register_style(
			$handler, $style['src'], $deps,
			$style['ver'],
			$style['media']
		);
	}

	/**
	 * A wrapper for registering scripts.
	 * 
	 * @since 1.0.0
	 * 
	 * @param  string       $handler The name of the script.
	 * @return boolean|void          If it gets the script then register it or false.
	 */
	public function wp_register_script( $handler ) {
		$script = $this->get_script( $handler );

		if ( ! $script ) {
			return;
		}

		$deps = (array) apply_filters( $handler . '--script', $script['deps'] );
		wp_register_script(
			$handler, $script['src'], $deps,
			$script['ver'], $script['in_footer']
		);
	}

	/**
	 * Returns all publicly enqueuable stylesheets.
	 * 
	 * @since  1.0.0
	 * @return array List of publicly enqueuable stylesheets.
	 */
	public function _get_public_style_all() {

		return (array) apply_filters( 'gs_teca_get_public_style_all', [
			'gs-swiper',
			'gs-tippyjs',
			'gs-font-awesome-5',
			'gs-magnific-popup',
			'gs-teca-public',
			'gs-bootstrap-grid',
			'gs-teca-divi-public'
		]);
	}

	/**
	 * Returns all publicly enqueuable scripts.
	 * 
	 * @since  1.0.0
	 * @return array List of publicly enqueuable scripts.
	 */
	public function _get_public_script_all() {
		return (array) apply_filters( 'gs_teca_get_public_script_all', [
			'gs-swiper',
			'gs-isotope',
			'gs-tippyjs',
			'gs-magnific-popup',
			'gs-images-loaded',
			'gs-teca-public',
			'gs-carousel-ticker'
		]);
	}

	/**
	 * Returns all admin enqueuable stylesheets.
	 * 
	 * @since  1.0.0
	 * @return array List of admin enqueuable stylesheets.
	 */
	public function _get_admin_style_all() {
		return (array) apply_filters( 'gs_teca_get_admin_style_all', [] );
	}

	/**
	 * Returns all admin enqueuable scripts.
	 * 
	 * @since  1.0.0
	 * @return array List of admin enqueuable scripts.
	 */
	public function _get_admin_script_all() {
		return (array) apply_filters( 'gs_teca_get_admin_script_all', [] );
	}

	public function _get_assets_all( $asset_type, $group, $excludes = [] ) {

		if ( ! in_array( $asset_type, [ 'style', 'script' ]) || ! in_array( $group, [ 'public', 'admin' ] ) ) {
			return;
		}

		$get_assets = sprintf( '_get_%s_%s_all', $group, $asset_type );
		$assets     = $this->$get_assets();

		if ( ! empty( $excludes ) ) {
			$assets = array_diff( $assets, $excludes );
		}

		return (array) apply_filters( sprintf( 'gs_teca_%s__%s_all', $group, $asset_type ), $assets );

	}

	public function _wp_load_assets_all( $function, $asset_type, $group, $excludes = [] ) {
		if ( ! in_array( $function, [ 'enqueue', 'register' ] ) || ! in_array( $asset_type, [ 'style', 'script' ] ) ) {
			return;
		}

		$assets   = $this->_get_assets_all( $asset_type, $group, $excludes );
		$function = sprintf( 'wp_%s_%s', $function, $asset_type );

		foreach( $assets as $asset ) {
			$this->$function( $asset );
		}
	}

	public function wp_register_style_all( $group, $excludes = [] ) {
		$this->_wp_load_assets_all( 'register', 'style', $group, $excludes );
	}

	public function wp_enqueue_style_all( $group, $excludes = [] ) {
		$this->_wp_load_assets_all( 'enqueue', 'style', $group, $excludes );
	}

	public function wp_register_script_all( $group, $excludes = [] ) {
		$this->_wp_load_assets_all( 'register', 'script', $group, $excludes );
	}

	public function wp_enqueue_script_all( $group, $excludes = [] ) {
		$this->_wp_load_assets_all( 'enqueue', 'script', $group, $excludes );
	}

	// Use to direct enqueue
	public function wp_enqueue_style( $handler ) {
		$style = $this->get_style( $handler );

		if ( ! $style ) {
			return;
		}

		$deps = (array) apply_filters( $handler . '--style-enqueue', $style['deps'] );
		wp_enqueue_style(
			$handler, $style['src'], $deps, $style['ver'], $style['media']
		);
	}

	public function wp_enqueue_script( $handler ) {
		$script = $this->get_script( $handler );

		if ( ! $script ) {
			return;
		}

		$deps = (array) apply_filters( $handler . '--script-enqueue', $script['deps'] );

		wp_enqueue_script(
			$handler, $script['src'], $deps,
			$script['ver'], $script['in_footer']
		);
	}

	public function print_plugin_icon_css() {
		?>
			<style>
				#adminmenu #toplevel_page_gs-the-events-calendar-addon > a.menu-top {
					display: flex;
					align-items: center;
				}

				#adminmenu #toplevel_page_gs-the-events-calendar-addon .wp-menu-image {
					display: flex;
					align-items: center;
					justify-content: center;
					float: none;
					width: 36px;
					min-height: 34px;
				}

				#adminmenu #toplevel_page_gs-the-events-calendar-addon .wp-menu-image img {
					display: block;
					width: 20px;
					height: 20px;
					padding: 0;
					margin: 0;
					opacity: 1;
					object-fit: contain;
				}

				#adminmenu #toplevel_page_gs-the-events-calendar-addon .wp-menu-name {
					padding: 8px 8px 8px 0;
				}

				.folded #adminmenu #toplevel_page_gs-the-events-calendar-addon .wp-menu-image {
					position: absolute;
					z-index: 25;
				}

				#adminmenu .wp-submenu li:nth-child(7):has( a[href^="admin.php?page=the-events-calendar-addon-pro"] ) {
					position: relative;
					margin-top: 16px;
				}
				
				#adminmenu .wp-submenu li:nth-child(7):has( a[href^="admin.php?page=the-events-calendar-addon-pro"] ):after {
					content: "";
					position: absolute;
					background: hsla(0, 0%, 100%, .2);
					width: calc(100%);
					height: 1px;
					top: -8px;
				}
			</style>
		<?php
	}

	/**
	 * Enqueue assets for the plugin based on all dep checks and only 
	 * if current page contains the shortcode.
	 * 
	 * @since  3.0.9
	 * @return void
	 */
	public function enqueue_scripts() {
		// Register Styles
		$this->wp_register_style_all( 'public' );
	
		// Register Scripts
		$this->wp_register_script_all( 'public' );

		add_action( 'wp_enqueue_scripts', function() {
			wp_enqueue_style( 'gs-teca-public' );
		}, 999 );

		$this->wp_enqueue_script( 'gs-magnific-popup' );
    	$this->wp_enqueue_style( 'gs-magnific-popup' );
		
		// Maybe enqueue assets
		gsTecaAssetGenerator()->enqueue( gsTecaAssetGenerator()->get_current_page_id() );

		do_action( 'gs_teca_assets_loaded' );
		
	}

	public function enqueue_admin_scripts( $hook ) {

		if ( $hook != 'events_page_sort_tribe_events' ) {
			return;
		}

		wp_enqueue_script( 'jquery-ui-sortable' );

		// Register Styles
		$this->wp_register_style_all( 'admin' );
		$this->wp_register_style_all( 'public' );

		// Register Scripts
		$this->wp_register_script_all( 'admin' );
		$this->wp_enqueue_style('gs-font-awesome-5');
	
	}

	public static function add_dependency_scripts( $handle, $scripts ) {

		global $wp_scripts;

		if ( empty($scripts) || empty($handle) ) return;
		if ( ! isset($wp_scripts->registered[$handle]) ) return;

		$wp_scripts->registered[$handle]->deps = array_unique( array_merge( $wp_scripts->registered[$handle]->deps, $scripts ) );

	}

	public static function add_dependency_styles( $handle, $styles ) {
		
		global $wp_styles;
		
		if ( empty($styles) || empty($handle) ) return;
		if ( ! isset($wp_styles->registered[$handle]) ) return;
		
		$wp_styles->registered[$handle]->deps = array_unique( array_merge( $wp_styles->registered[$handle]->deps, $styles ) );

	}

}
