...
final class Term_Order {
	 ...
	 */
	public $db_version = 202507240001;
	...
	public function init() {
		...
		// Ajax actions
		add_action( 'wp_ajax_gs_teca_reordering_terms', array( $this, 'ajax_reordering_terms' ) );
	...
	public function enqueue_scripts() {
		wp_enqueue_script( 'gs-teca-term-order-quick-edit', $this->url . 'includes/term-order/assets/js/quick-edit.js', array( 'jquery' ), $this->db_version, true );
 ...
 * @since 0.1.0
 */
function _term_order() {
	static $term_order = null;

	if ( is_null( $term_order ) ) {
		$term_order = new Term_Order();
		$term_order->init();
	}

	return $term_order;
}
add_action( 'init', 'GS_TECA\_term_order', 99 );
